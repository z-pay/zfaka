	...
	public function pay($payconfig,$params)
	{
		try{
			...
			if(is_array($result) AND isset($result['response']['qr_code']) AND $result['response']['qr_code']){
				$result_params = array('type'=>0,'subjump'=>0,'paymethod'=>$this->paymethod,'qr'=>"/product/order/showqr/?url=".urlencode($result['response']['qr_url']),'payname'=>$payconfig['name'],'overtime'=>$payconfig['overtime'],'money'=>$params['money']);
